...
DATA_PATH  = os.path.join(SCRIPT_DIR, "..", "Data", "analysis_merged_subject_level.csv")
...
try:
    table2 = pd.read_csv('results_table2_hyperparameters.csv')
...
mcnemar_df.to_csv('model_distinguisher_results.csv', index=False)
...
plt.tight_layout()
plt.savefig('plot_calibration_curves.png', dpi=200, bbox_inches='tight')
plt.close()
...
plt.tight_layout()
plt.savefig('plot_logreg_coefficients.png', dpi=200, bbox_inches='tight')
plt.close()
...
plt.tight_layout()
plt.savefig('plot_permutation_importance.png', dpi=200, bbox_inches='tight')
plt.close()
