...
OUTPUT_DIR  = BASE_DIR / "PCA+KNN-Emma" / "Outputs" / "race"
...
y = le_race.fit_transform(df["RACEGRP"].values)
race_labels = le_race.classes_
print(f"  Race classes: {dict(zip(le_race.classes_, le_race.transform(le_race.classes_)))}")
n_samples = len(df)
...
for i, race in enumerate(race_labels):
    count = (y == i).sum()
    print(f"  {race:<20}: {count}  ({100*count/n_samples:.1f}%)")
...
race_colors = ["#E87D7D", "#6BAED6", "#5BAD72", "#F4A460", "#9B59B6"]
for i, race in enumerate(race_labels):
    mask = y == i
    ax_p.scatter(X_global_pca[mask, 0], X_global_pca[mask, 1],
                 c=race_colors[i % len(race_colors)],
                 alpha=0.7, s=40, label=race)
...
# ================================================================================
n_classes = len(race_labels)
cm_best   = np.zeros((n_classes, n_classes), dtype=int)
all_true, all_pred = [], []
...
cm_best  = sk_cm(all_true, all_pred)

metrics  = {
    "Accuracy":          round(accuracy_score(all_true, all_pred), 4),
    "Balanced Accuracy": round(accuracy_score(all_true, all_pred), 4),
    "F1 Score":          round(f1_score(all_true, all_pred, average="weighted"), 4),
    "Sensitivity":       0.0,
    "Specificity":       0.0,
}
...
fig.suptitle(
    f"Cytokines-Only PCA+KNN — Race Prediction\nBest Config: PCA={best_key[0]}, K={best_key[1]}\n"
...
ax_c.set_xticks(range(len(race_labels)))
ax_c.set_xticklabels([f"Pred {r[:4]}" for r in race_labels], fontsize=7, rotation=45)
ax_c.set_yticks(range(len(race_labels)))
ax_c.set_yticklabels([f"Act {r[:4]}" for r in race_labels], fontsize=7)
