...
OUTPUT_DIR  = BASE_DIR / "XGBoost" / "cytokine-only"/"age"/ "Outputs" / "pca-on-age-subject-dropped"
...
# Log1p transform first to reduce skew, then robust scale
...
print(f"Class distribution: Young={y.sum()==0} | "
      f"Young={( y==0).sum()} Older={( y==1).sum()}")
...
# F-scores and p-values per cytokine
...
print(f"\nTop 5 cytokines by F-score (age discrimination):")
...
xgb = XGBClassifier(
    n_estimators=100,
    max_depth=3,          # shallow to prevent overfitting at n=125
    learning_rate=0.05,
    subsample=0.8,
...
cv_acc_xgb = cross_val_score(xgb, X_selected, y, cv=cv_rep, scoring='accuracy')
cv_f1_xgb  = cross_val_score(xgb, X_selected, y, cv=cv_rep, scoring='f1')
cv_auc_xgb = cross_val_score(xgb, X_selected, y, cv=cv_rep, scoring='roc_auc')
...
# Feature importance
xgb_fit = XGBClassifier(n_estimators=100, max_depth=3, random_state=RANDOM_STATE,
...
# ============================================================================
# STEP 7: METHOD 3 — LOGISTIC REGRESSION (recommended for binary + small n)
# ============================================================================
print(f"\n[7/7] METHOD 3: LOGISTIC REGRESSION (L2 regularized)...")

lr = LogisticRegression(
    penalty='l2',
    C=0.1,              # stronger regularization for small n
    max_iter=1000,
    random_state=RANDOM_STATE,
...
# LR coefficients — which cytokines push toward Older
...
comparison.to_csv('./results_binary_age_comparison.csv', index=False)
...
# Panel 5: LR coefficients — which cytokines predict older age
...
# Panel 6: ANOVA F-scores — cytokine discrimination power
