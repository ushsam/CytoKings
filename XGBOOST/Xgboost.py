...
df = pd.read_csv(DATA_PATH)
print(f"Data loaded: {df.shape[0]} samples x {df.shape[1]} columns")
...
print(f"Target   : SEXC  (Male=1, Female=0)")
print(f"Females  : {(y==0).sum()}   Males: {(y==1).sum()}")
...
print("Clipped all features to 99th percentile")
...
print("Applied RobustScaler (handles outliers better than Z-score)")
...
print(f"Feature/Sample ratio: {12}/{X.shape[0]} = {12/X.shape[0]:.3f}")
print("\nTop 5 selected features:")
print(feature_selection_df[feature_selection_df['Selected']].head(5).to_string(index=False))
 
feature_selection_df.to_csv(OUTPUT_DIR / 'sex_feature_selection_anova.csv',
                             index=False)
print("\nSaved: sex_feature_selection_anova.csv")
 
# ============================================================================
# 5a. GRID SEARCH — XGBoost hyperparameters
# ============================================================================
print("\n" + "=" * 80)
...
for n_est in xgb_param_grid['n_estimators']:
    for depth in xgb_param_grid['max_depth']:
        for lr in xgb_param_grid['learning_rate']:
            ...
            pipe = Pipeline([
                ('selector', SelectKBest(f_classif, k=12)),
                ('xgb', XGBClassifier(n_estimators=n_est, max_depth=depth,
            ...
            print(f"  [{combo_idx}/{total_combos}] n_est={n_est}, depth={depth}, "
                  f"lr={lr}  ->  {mean_acc:.4f} +/- {scores.std():.4f}")
...
grid_df = pd.DataFrame(grid_results).sort_values('CV_Accuracy_Mean',
                                                   ascending=False)
grid_df.to_csv(OUTPUT_DIR / 'results_xgboost_config_comparison.csv', index=False)
...
# ============================================================================
# 5b. GRID SEARCH — Logistic Regression C tuning
# ============================================================================
print("\n" + "=" * 80)
...
pd.DataFrame(lr_results).to_csv(OUTPUT_DIR / 'results_logreg_c_tuning.csv',
                                 index=False)
...
print(f"\nUsing RepeatedStratifiedKFold (5 folds x 10 repeats = 50 total folds)")
...
# ============================================================================
print("\n" + "=" * 80)
print("CROSS-VALIDATION RESULTS (10x5-fold)")
print("=" * 80)
...
feature_importance_df.to_csv(OUTPUT_DIR / 'sex_xgboost_feature_importance.csv',
                              index=False)
print("\nSaved: sex_xgboost_feature_importance.csv")
 
# ============================================================================
# 9. FINAL MODEL EVALUATION (on full data — for reference only)
# ============================================================================
print("\n" + "=" * 80)
print("FINAL XGBOOST EVALUATION (training set — use CV metrics for reporting)")
print("=" * 80)
...
print(f"ROC-AUC   : {roc_auc_final:.4f}  (NOTE: training-set AUC — inflated)")
print("\nConfusion Matrix:")
print(f"  TN (Correctly classified Females) : {cm[0, 0]}")
print(f"  FP (Females misclassified)        : {cm[0, 1]}")
print(f"  FN (Males misclassified)          : {cm[1, 0]}")
print(f"  TP (Correctly classified Males)   : {cm[1, 1]}")
...
models_sorted = sorted(cv_results.items(),
...
colors  = ['#FF6B6B' if m[0] == 'XGBoost' else '#4ECDC4'
           for m in models_sorted]
ax1.barh(model_names, accuracies, color=colors)
ax1.set_xlabel('Accuracy')
ax1.set_title('Model Comparison: CV Accuracy (Sex Classification)',
...
ax2.barh(top_features['Cytokine'], top_features['Importance'],
         color=colors_feat)
ax2.set_xlabel('Importance Score')
ax2.set_title('Top 10 Most Important Cytokines (XGBoost)',
              fontsize=11, fontweight='bold')
ax2.invert_yaxis()
...
# 4. CV-based ROC curve (correct — not training set)
...
ax5.bar(['Female', 'Male'], class_counts,
...
n_sel = int(mask.sum())
ax7.pie([n_sel, len(cytokines) - n_sel],
        labels=[f'Selected\n({n_sel})',
                f'Not Selected\n({len(cytokines) - n_sel})'],
        autopct='%1.1f%%', colors=['#90EE90', '#FFB6C6'], startangle=90)
ax7.set_title(f'ANOVA Feature Selection ({n_sel}/{len(cytokines)} features)',
              fontsize=11, fontweight='bold')
 
# 8. All Models Accuracy Bar
ax8 = fig.add_subplot(gs[2, 1:])
ax8.bar(model_names, accuracies,
        color=['#FF6B6B' if m == 'XGBoost' else '#4ECDC4'
               for m in model_names])
ax8.set_ylabel('Score')
ax8.set_xlabel('Model')
ax8.set_title('Cross-Validated Accuracy by Model',
              fontsize=11, fontweight='bold')
ax8.set_ylim(0.4, 0.8)
ax8.grid(axis='y', alpha=0.3)
plt.setp(ax8.xaxis.get_majorticklabels(), rotation=45, ha='right')
 
plt.savefig(FIGURES_DIR / 'final_plots_sex_classification_comprehensive_results.png',
...
top_anova         = feature_selection_df[feature_selection_df['Selected']]\
                        .sort_values('ANOVA_F_Score', ascending=False)
top_cytokines_plot = top_anova['Cytokine'].tolist()[:8]
 
fig2, axes2 = plt.subplots(3, 3, figsize=(18, 15))
fig2.suptitle('Cytokine Differences Between Sexes (Healthy Adults)',
              fontsize=15, fontweight='bold', y=1.01)
 
# Panel 1: ANOVA F-scores
ax = axes2[0, 0]
...
ax.set_title('ANOVA F-Scores: All 18 Cytokines\n(red = selected for model)',
             fontsize=10, fontweight='bold')
ax.axvline(fs_sorted[fs_sorted['Selected']]['ANOVA_F_Score'].min(),
...
ax.set_title('Mean Cytokine Levels by Sex\n(Z-scored, ANOVA-selected)',
             fontsize=10, fontweight='bold')
ax.set_xlabel('')
 
# Panel 3: Log2 fold change
ax          = axes2[0, 2]
eps         = 1e-6
...
ax.set_title('Cytokine Log2 Fold Change\nMale vs Female',
             fontsize=10, fontweight='bold')
ax.text(0.98, 0.02,
        'Blue = higher in Males\nRed = higher in Females',
        transform=ax.transAxes, fontsize=8, ha='right', va='bottom',
        bbox=dict(boxstyle='round', facecolor='white', alpha=0.7))
 
# Panels 4-9: Violin plots for top 6 cytokines
...
for i, cyto in enumerate(top_cytokines_plot[:6]):
    ...
    f_val = feature_selection_df.loc[
        feature_selection_df['Cytokine'] == cyto, 'ANOVA_F_Score'
    ].values[0]
...
plt.tight_layout()
plt.savefig(FIGURES_DIR / 'final_plots_sex_cytokine_differences.png',
            dpi=300, bbox_inches='tight', facecolor='white')
plt.close()
...
results_summary = {
    ...
    'XGBoost_Train_ROC_AUC':   f"{roc_auc_final:.4f} (inflated — training set)",
...
print(f"\n  Dataset  : {X.shape[0]} healthy adults  "
      f"(Females: {np.sum(y==0)}  Males: {np.sum(y==1)})")
...
print(f"  Model Performance (10x5-fold CV):")
...
for idx, row in feature_importance_df.head(3).iterrows():
    print(f"    {row['Cytokine']} ({row['Importance']:.3f})")
 
# ============================================================================
# 12. CV-BASED ROC CURVE (correct — not training set)
# ============================================================================
print("\n" + "=" * 80)
print("CV-BASED ROC CURVE")
print("=" * 80)
...
y_proba_cv  = cross_val_predict(
    ...
    X_selected, y, cv=cv_roc, method='predict_proba'
)[:, 1]
...
plt.tight_layout()
plt.savefig(FIGURES_DIR / 'results_xgboost_cv_roc.png',
...
if SHAP_AVAILABLE:
    ...
    plt.title("SHAP Summary: Feature Contributions to Sex Prediction\n"
              ...
              fontsize=10, fontweight='bold')
    plt.tight_layout()
    plt.savefig(FIGURES_DIR / 'results_shap_beeswarm.png',
                dpi=200, bbox_inches='tight', facecolor='white')
    plt.close()
    ...
    plt.title("SHAP Feature Importance (Mean |SHAP|)",
              fontsize=10, fontweight='bold')
    plt.tight_layout()
    plt.savefig(FIGURES_DIR / 'results_shap_bar.png',
                dpi=200, bbox_inches='tight', facecolor='white')
    plt.close()
    ...
    shap_mean_abs = pd.DataFrame({
        ...
        'Direction':     ['Male_up' if v > 0 else 'Female_up'
                          for v in shap_values.mean(axis=0)]
    }).sort_values('Mean_Abs_SHAP', ascending=False)
 
    shap_mean_abs.to_csv(OUTPUT_DIR / 'results_shap_values.csv', index=False)
    ...
    print("\nInterpretation:")
    print("  Male_up   = high cytokine value pushes prediction toward Male")
    print("  Female_up = high cytokine value pushes prediction toward Female")
else:
    print("SHAP not available. Install with: pip install shap")
 
# ============================================================================
# 14. HYPERPARAMETER TUNING SUMMARY (Table 2)
# ============================================================================
print("\n" + "=" * 80)
...
table2_df = pd.DataFrame([
    {'Model': 'PCA+KNN', 'Parameter': 'n_pca',
     'Search Range': '2-10', 'Best Value': '4',
     'CV Score': '56.8%'},
    {'Model': 'PCA+KNN', 'Parameter': 'K',
     'Search Range': '3-7', 'Best Value': '7',
     'CV Score': '56.8%'},
    {'Model': 'XGBoost', 'Parameter': 'n_estimators',
     'Search Range': '{100, 200}',
     'Best Value': str(best_xgb_params['n_estimators']),
     'CV Score': f"{best_xgb_score*100:.1f}%"},
    {'Model': 'XGBoost', 'Parameter': 'max_depth',
     'Search Range': '{3, 5}',
     'Best Value': str(best_xgb_params['max_depth']),
     'CV Score': f"{best_xgb_score*100:.1f}%"},
    {'Model': 'XGBoost', 'Parameter': 'learning_rate',
     'Search Range': '{0.05, 0.1}',
     'Best Value': str(best_xgb_params['learning_rate']),
     'CV Score': f"{best_xgb_score*100:.1f}%"},
    {'Model': 'LogReg',  'Parameter': 'C',
     'Search Range': '{0.01, 0.1, 1, 10}',
     'Best Value': str(best_lr_C),
     'CV Score': f"{best_lr_score*100:.1f}%"},
])
 
print(table2_df.to_string(index=False))
table2_df.to_csv(OUTPUT_DIR / 'results_table2_hyperparameters.csv', index=False)
print("\nSaved: results_table2_hyperparameters.csv  <- copy into Table 2")
 
if MLFLOW_AVAILABLE:
    for fname in ['sex_model_comparison.csv',
                  'sex_xgboost_feature_importance.csv',
                  'results_shap_values.csv',
                  'results_table2_hyperparameters.csv']:
